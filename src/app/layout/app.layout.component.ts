...
import { findPassesByStateAndDate } from '../store/Passe/Passe.action';
...
export class AppLayoutComponent implements OnDestroy, OnInit {
    ...
    usedConditionDialogShow: boolean = false;
    aboutDialogShow: boolean = false;
    ...
    constructor(
        ...
        private store: Store,
        private storeDialog: Store<{ showDialog: DialogModel }>,
    ) {
    ...
    ngOnInit() {
        if(localStorage.getItem('userdata') != null ) {
            ...
            this.tokenReceived = JSON.parse(jsonstring) as AuthResponse;
            const today = getTodayDate();            
            if(this.tokenReceived.role === "Agent"){
            this.store.dispatch(findPassesByStateAndDate({agent: this.tokenReceived.nom as string, nom: "Nouveau", date: today}));
            } else {
                this.store.dispatch(getCategories());
                this.store.dispatch(getStatuts());
            }
        }
        this.storeDialog.select('showDialog').subscribe( data => {
            this.usedConditionDialogShow = data.showUsedCondition;
            this.aboutDialogShow = data.showApropos;
        })
    }

    toggleDialogUsedCondition(){
        this.store.dispatch(usedConditionDialog());
    }
    toggleDialogAbout(){
        this.store.dispatch(aboutDialog());
    }
